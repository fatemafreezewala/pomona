import { StyleSheet, Text, View ,Image, ScrollView,FlatList, TouchableOpacity,Modal} from 'react-native'
...
const RestaurantDetail = ({route,navigation}) => {
  ...
  const pillData = [ 
    {id:1,img:<Home></Home>,title:item.restaurant_type},
    {id:2,img:<Salad></Salad>,title:item.restaurant_Cuisine},
...
const onShareRestro = () =>{
  ...
  return (
    ...
    <View style={[globalStyle.rowSpaceBetween,{height:50,paddingRight:10,backgroundColor:'#fff'}]}>
    ...
    <ScrollView>
      ...
      {item.restaurant_LogoUrl != '' ?
       (<Image style={{width:'100%',height:180}} source={{uri:imageApi+item.restaurant_LogoUrl}}></Image>) : 
      (<Image style={{alignSelf:'center'}} source={require('../../assets/img/logo.png')}></Image>) }
      
      <Text style={[styles.boxhead,{fontSize:18,margin:'3%'}]}>{item.restaurant_name}</Text>
      ...
      <View style={styles.box}>
        ...
        <View style={[globalStyle.rowSpaceBetween,globalStyle.mt5]}>
          ...
          <TouchableOpacity onPress={()=>navigation.navigate('Website',{url:item.restaurant_website})}>
          <Text style={[styles.desp,{color:colors.linkBlue,textDecorationLine:'underline'}]}>
            {item.restaurant_website}</Text>
          </TouchableOpacity>
      ...
      <View style={styles.box}>
        <View style={globalStyle.rowCenter}>
         <Loc></Loc>
          <Text style={{color:colors.linkBlue,fontFamily:fontFamily.Medium,width:'90%',marginLeft:15}}>
          {item.restaurant_address},{item.restaurant_address_city},{item.restaurant_address_state},{item.restaurant_country}
          </Text>
        </View>
        <View> 
          ...
          <Text style={{color:colors.linkBlue,fontFamily:fontFamily.Regular,marginLeft:15,textAlign:'center'}}>
          View on map
        </Text>
          </TouchableOpacity>
     ...
     {item.restaurant_created_by == 'admin' ? (<>
      <View style={{flexDirection:'row',alignItems:'center'}}>
        ...
        <Text style={[styles.boxhead,globalStyle.ml5,{fontSize:14}]}>Pomona App Admin</Text>
      </View>
     </>) : (<>
      <View style={{flexDirection:'row',alignItems:'center'}}>
      <Image style={{width:60,height:60,borderRadius:60}} source={{uri:imageApi+item.u_profile}}></Image>
      <Text style={[styles.boxhead,globalStyle.ml5,{fontSize:14}]}>{item.u_profilename}</Text>
    </View>
    <Divider style={[globalStyle.mt5,globalStyle.mb5]}></Divider>
      <Text style={styles.boxhead}>Author Note - </Text>
      <Text style={styles.desp}>{item.restaurant_allergyCommitmentMessage}</Text></>)}
      </View>
