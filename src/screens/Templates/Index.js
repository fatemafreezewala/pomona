...
const Index = ({route,navigation}) => {
  ...
  const items = ({item}) =>{
    return <Card 
    onPress={()=> navigation.navigate('TemplateDetails',{item:item,user:user})} item={item}></Card>
  } 
  return (
    <View style={[globalStyle.container,globalStyle.dullBackground]}>
      <InnerPageHeading title="MY TEMPLATES" subtitle="Create and maintain your allergy cards"></InnerPageHeading>
     ...
     showsVerticalScrollIndicator={false}
      ...
      data={getTemplate }
      renderItem={items}
      keyExtractor={(item)=>item.user_template_id}
      ></FlatList>

     <View style={{alignSelf:'flex-end',position:'absolute',bottom:0,margin:15,right:5}}>
