...
const GetInTouch = ({navigation}) => {
  ...
  const getFAQ = () =>{
    setLoading(true)
    api.get('?action=getFAQ').then(res=>{
  ...
  const items = ({item}) =>{
    return  <Collapse style={{backgroundColor:'#fff',borderRadius:10,marginTop:10}}> 
          <CollapseHeader>
            <View style={[{backgroundColor:'#fff',padding:15,borderRadius:10},globalStyle.rowSpaceBetween]}>
                <View style={globalStyle.rowCenter}>
                
                    <Text style={{color:'#000',fontFamily:fontFamily.Medium}}>{item.faq_title}</Text>
