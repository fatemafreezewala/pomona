...
const [country, setCountry] = React.useState(
    { "cca2":user.u_cca,"callingCode":[user.u_country_code]})
...
const handleInput = (key,val) => {
  ...
  const onSuubmit = async() =>{
    ...
    if(userDetails.u_email != '' && userDetails.u_profile != '' && 
    userDetails.u_fullname != '' && 
    country.cca2 != '' && userDetails.u_phone != '' && userDetails.u_password != '' && userDetails.u_profilename != ''){
        setLoading(true)
  ...
  return (
    <>
    <View style={[{height:50,flexDirection:'row',alignItems:'center'},globalStyle.dullBackground]}>
    ...
    <View style={[globalStyle.container,globalStyle.dullBackground]}>
        <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {image != '' && (<Image 
        style={{width:130,height:120,borderRadius:10}} source={{uri:image}}></Image>)}
            {image == '' && (
                <>
            {user.u_profile != '' ? (<Image style={{width:130,height:120,borderRadius:10}} source={{uri:imageApi+user?.u_profile}}></Image>) : (<Photo width={120} height={120}></Photo>)}
                </>
            )}
            <TouchableOpacity style={{padding:5}} onPress={()=>{
                UploadPic()
            }}>
            <FA5 size={15} color={colors.secondry} 
            style={{borderRadius:15,position:'absolute',right:0,bottom:0,backgroundColor:'#fff',padding:10}} name="edit"></FA5>
            </TouchableOpacity>
            </View>
            ...
            <Text style={{marginBottom:5,fontFamily:fontFamily.Regular,color:colors.black}}>Full Name</Text>
            <CTextInput onChange={(e)=>{
                handleInput('u_fullname',e)
            }} defaultValue={user.u_fullname} placeholder="Full name"></CTextInput>
            <View style={[globalStyle.mt5]}></View>
            <Text style={{marginBottom:5,fontFamily:fontFamily.Regular,color:colors.black}}>Profile Name</Text>
            <CTextInput onChange={(e)=>{
                handleInput('u_profilename',e)
            }} defaultValue={user.u_profilename} placeholder="Profile name"></CTextInput>
            <View style={[globalStyle.mt5]}></View>
            <Text style={{marginBottom:5,fontFamily:fontFamily.Regular,color:colors.black}}>Email</Text>
            <CTextInput onChange={(e)=>{
                handleInput('u_email',e)
            }} defaultValue={user.u_email}></CTextInput>
            <View style={[globalStyle.mt5]}></View>
            <Text style={{marginBottom:5,fontFamily:fontFamily.Regular,color:colors.black}}>Phone</Text>
        <CountryPicker country={country} setCountry={(e)=>{
            setCountry(e)
            ...
            <View style={{marginHorizontal:15,marginTop:10,borderRadius:15}}>
                <View style={globalStyle.rowSpaceBetween}>
                    <Text style={{color:'#000',fontFamily:fontFamily.Medium,fontSize:16}}>password & Security</Text>
                    <Arrow></Arrow>
                </View>
                <TouchableOpacity onPress={()=>{
                    navigation.navigate('ChangePassword',{user:userDetails})
                }}>
                <Text style={[globalStyle.paragraph,{color:colors.secondry},globalStyle.mt5]}>      
            ...
            <View style={{marginHorizontal:15,marginTop:10,borderRadius:15}}>
                <View style={globalStyle.rowSpaceBetween}>
                    <Text style={{color:'#000',fontFamily:fontFamily.Medium,fontSize:16}}>Close Account</Text>
                </View>
                
                <Text style={[globalStyle.paragraph,{color:colors.black},globalStyle.mt5]}>      
            ...
            <Button loading={loading} onPress={()=>{
                onSuubmit()
                }} style={[globalStyle.w95,globalStyle.mt10,{backgroundColor:colors.secondry}]} title="Save Changes"></Button>
